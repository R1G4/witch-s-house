...
void thirdSnakeDead::release()
{
}

void thirdSnakeDead::update()
{
	camera.x = _player->getPlayerLocX();
	camera.y = _player->getPlayerLocY();
	...
	_deadManager->setLocXY(WINSIZEX / 2 + 200, WINSIZEY / 2);
	_deadManager->setDead(DEAD_SNAKE);
	_deadManager->update();
...
void thirdSnakeDead::tileCollision()
{
	for (int i = 0; i < TILEY; i++)
	{
		for (int j = 0; j < TILEX; j++)
...
void thirdSnakeDead::Collision()
{
}

void thirdSnakeDead::load()
{
	HANDLE file;
	DWORD read;
	file = CreateFile("Stage/3f_snake_dead.map", GENERIC_READ, NULL, NULL,
