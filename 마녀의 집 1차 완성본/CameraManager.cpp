...
void CameraManager::txtRender(string str, Vector2 vec, int size, const D2DRenderer::DefaultBrush & defaultbrush, const DWRITE_TEXT_ALIGNMENT & align, const wstring & font)
{
	string strtemp = str;
	wstring str_w;
	str_w.assign(strtemp.begin(), strtemp.end());
	_D2DRenderer->RenderText(vec.x - cameraX, vec.y - cameraY, str_w, size, defaultbrush, align, font);
}
