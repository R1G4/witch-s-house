...
void garDen::render()
{
	CAMERAMANAGER->render(_backGround, Vector2(720, 648));
	for (int i = 0; i < TILEY; i++)
	{
		for (int j = 0; j < TILEX; j++)
		{
...
void garDen::load()
{
	HANDLE file;
	DWORD read;
	file = CreateFile("Stage/saveMap1.map", GENERIC_READ, NULL, NULL,
	...
	for (int i = 0; i < TILEY; i++)
	{
		for (int j = 0; j < TILEX; j++)
		{
			if (_tiles[i*TILEY + j].frameKeyName == "violaIdle")_player->setStart(PointMake(j, i));
		}
	}
	camera = _player->getPlayerLoc();
	CloseHandle(file);
